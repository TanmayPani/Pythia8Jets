...
void PythiaTest(int PtHatLow = 20, int PtHatHigh = 25, string decay_switch = "off", bool StoreFullEvent = false) {
	...
	TStirng *outname = new TString();

	TFile *f = new TFile(Form("Pythia8_STARTune_pSet17_PtHat%i_%i_WkDec%s_%s.root", PtHatLow, PtHatHigh, decay_switch.c_str(), EventStorage.c_str()), "RECREATE");
